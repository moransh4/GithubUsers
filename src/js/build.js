...
var vnode,
 ...
 errorIndex= 0,   // error key
  ...
  user={
    ...
    /**
     *
     * @param data : array of errors or array of data of the users
     * @returns {*} new list display to the user
     */
  function view(data) {
    return h('div',
        [
          ...
          h('div.list', { style: { height: totalHeight + 'px' } },
              findUser===true ?  data.map(userView) : data.map(errorView))]);  // display all the users or the error to the view
  }



    /**
     *  listener to  input of the user and display it on the p.name
     * @param e : input
     */
  ...
  var enterUser = function (e)  {
    ...
    /**
     *
     *  When user click on button "Fetch!" - he get via fetch (same as AJAX) the json file of the user he type
     *  and for this - to gdet the json we want ,we set the GitHub api url with the key username that the user type in
     *
     *
     */
  function add() {
    ...
    /**
     * get the json file of the user's followers
     * based on api url with the username param
     * and get the amount of followers
     */
  ...
  //}
    ...
    /**
     * initialize the findUser to false , parse the json file to object , and search under array objects of items
     * if the key [login] equal to the input the user enter .
     * if we found the user , findUser turn to true ,and  search how much followers the user have .
     * then we add to data[] the user details we get from json file.
     *
     * @param text :  'Json' file of the user
     */
  ...
  function search_user(text){
    ...
    for (var i=0 ; i<users.length ; i++){
      if(users[i].login === user.name){
          //need search for double user - and not add to data[]
        findUser = true;
        console.log(users[i]);
        searchFollowers();
          console.log("Followers:"+user.followers);
    ...
    /**
     * if we don't found the user , add to errors array the detaills -
     * username , hight of the element , and the id (index) of the error
     */
  ...
  function error(){
    ...
    /**
     * when user click on X , trigger the remove function.
     * return filter array of errors or data(users) - and display it in the view
     *
     * @param user : user / error to delete from the array
     */
...
function remove(user) {  //not delete the last user
  ...
  render();
    ...
    /**
     * view of error
     * @param error : errors at index i - errors[i]
     * @returns {*} the view of the particular error
     */
  function errorView(error){
    ...
    /**
     * view of user
     * @param user : data at index i - data[i]
     * @returns {*} the view of the particular data(user)
     */
function userView(user) {
  ...
  return h('div.row', {
    ...
    /**
     * when user click on the smile img - it change its state appearance.
     * @param e :  img element
     */
function likeUser(e){
  ...
  e.target.src = "../img/face.png" ? ("../img/smiling.png") : ("../img/face.png") ;
     ...
     *  add  height and margin to the element and calculate the total height of the elements (data/errors element)
     * choose what to display based on the var findUser
     * (if founduser===true we display the user view if not found display the error view)
      */
function render() {

  if (findUser===true)
  {
    data = data.reduce(function (acc, m) {  //acc= previousValue , m = currentValue
  ...
  vnode = findUser===true ?
    ...
    /**
     * listener when DOM content loaded -
     * get the element of the container div and dispaly the view we wand
     * depends the findUser value .
     */
